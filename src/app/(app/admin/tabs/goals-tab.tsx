...
export function GoalsTab() {
    ...
    const handleUpdateGoal = (goal: Goal) => {
        setGoals(goals); // This triggers the save in context
    ...
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                {goals.map(goal => (
                    <div key={goal.id} className="grid grid-cols-1 sm:grid-cols-2 md:flex md:items-end gap-4 p-4 border rounded-lg">
                        <div className="flex-1">
                            <Label htmlFor={`goal-type-${goal.id}`}>Tipo</Label>
                            <Input id={`goal-type-${goal.id}`} value={getGoalLabel(goal.type)} readOnly />
                        </div>
                        <div className="flex-1">
                            <Label htmlFor={`goal-target-${goal.id}`}>Meta</Label>
                            <Input 
                        ...
                        <div className="flex-1">
                            <Label htmlFor={`goal-achieved-${goal.id}`}>Alcançado</Label>
                            <Input 
                        ...
                        <Button onClick={() => handleUpdateGoal(goal)} className="w-full md:w-auto">Salvar Meta</Button>
                    </div>
                ))}
            </CardContent>
        </Card>
